...
9. **Every installer behavior change ships with tests** — new artifact types, manifest fields, lint rules, copy/merge/uninstall semantics, CLI flags: each gets deterministic coverage in `tests/test_install.py` before it merges (happy path + the guard rails, e.g. "never overwrites", "lint rejects X"). Prompt-only artifacts (agents/skills/rules content) are exempt, but their *registration* is guarded both ways: `test_real_manifest_lints_clean` checks every manifest entry points at a real file, and `test_every_skill_dir_on_disk_is_registered` + `test_every_rule_agent_hook_file_on_disk_is_registered` check the reverse — every skill dir / rule / agent / hook file on disk is in the manifest, so a new one can't be silently dropped (rule 4).
